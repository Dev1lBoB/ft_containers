...
#define		LAST Last
		...
		public:
			...
			typedef		ft::MapIterator
						<key_type, value_type, Node>			iterator;
			typedef		ft::ReverseMapIterator
...
//
			...
			Node	*findNode(Node *node, const key_type &k) const
			{
				...
				if (Size == 0 || node->Flag != 0)
					return nullptr;
				if (Comp(k, node->Pair.first) == 0 && Comp(node->Pair.first, k) == 0)
					return node;
			...
			Node *insertNode(const key_type k, Node *node, const value_type &pair)
			{
				...
				if (Comp(k, node->Pair.first) && node->Left->Flag == 0)
					return insertNode(k, node->Left, pair);
				if (!Comp(k, node->Pair.first) && node->Right->Flag == 0)
					return insertNode(k, node->Right, pair);

				Node *newNode = createNode(pair);

				if (Comp(k, node->Pair.first))
					node->Left = newNode;
				else
					node->Right = newNode;
				newNode->Parent = node;
			...
			void deleteNodeWithBothChildren(Node *nodeToDelete)
			{
				...
				( pair< key_type, mapped_type> &)(nodeToDelete->Pair);
				pair< key_type, mapped_type > & b =
				( pair< key_type, mapped_type> &)(nodeToReplace->Pair);
				std::swap(a, b);
...
//
			...
			{
				LAST = createNode(pair<const key_type, mapped_type>());
				LEAF = createNode(pair<const key_type, mapped_type>());
				LAST->Flag = 1;
				LAST->Left = LEAF;
				LAST->Right = LEAF;
				LAST->Parent = nullptr;
...
//
			...
			mapped_type	&operator[](const key_type& k)
			{
				...
				if ((temp = findNode(ROOT, k)) != nullptr)
					return temp->Pair.second;
				value_type newPair =  makePair<const key_type, mapped_type>(k, mapped_type());
				return insertNode(k, ROOT, newPair)->Pair.second;
			}
...
//
			...
			{
				Node *temp = findNode(ROOT, val.first);
				if (temp)
					return ft::pair<iterator, bool>(iterator(temp), false);
				temp = insertNode(val.first, ROOT, val);
			...
			iterator insert (iterator position, const value_type& val)
			{
				...
				if (temp)
					return temp;
				return insertNode(val.first, ROOT, val);
			}
			...
			void insert (InputIterator first, InputIterator last, char (*)[sizeof(*first)] = NULL)
			{
				...
				{
					temp = findNode(ROOT, i->first);
					if (!temp)
						insertNode(i->first, ROOT, *i);
				}
			}

			void erase (iterator position)
			{
				Node *temp = findNode(ROOT, position->first);
				if (temp)
					deleteNode(temp);
			}
...
//
			...
			iterator lower_bound (const key_type& k)
			{
				...
				{
					iterator iter = begin();
					while (iter != LAST && Comp(k, iter->first) <= 0 && Comp(iter->first, k) >= 0)
						++iter;
					return iter;
				}
				return temp;
...
//
	...
	template <class Key, class T, class Compare, class Alloc>
		...
		const Map<Key,T,Compare,Alloc>& rhs )
		{
			...
			typename Map<Key, T>::const_iterator lhs_e = lhs.end();
			typename Map<Key, T>::const_iterator rhs_b = rhs.begin();
			typename Map<Key, T>::const_iterator rhs_e = rhs.end();

			while (lhs_b != lhs_e && rhs_b != rhs_e)
			{
				if (lhs_b->first != rhs_b->first || lhs_b->second != rhs_b->second )
					return false;
				lhs_b++;
				rhs_b++;
			}
	...
	template <class Key, class T, class Compare, class Alloc>
		...
		const Map<Key,T,Compare,Alloc>& rhs )
		{
			...
			while (lhs_b != lhs_e && rhs_b != rhs_e)
			{
				if ((lhs_b->first > rhs_b->first) || (lhs_b->first == rhs_b->first && lhs_b->second > rhs_b->second) )
					return true;
				lhs_b++;
				rhs_b++;
			}
